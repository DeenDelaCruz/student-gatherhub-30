...
/**
 * Gets the number of users interested in an event
...
export const getEventInterestCount = async (eventId: string): Promise<number> => {
  try {
    ...
    console.error("Unexpected error getting event interest count:", error);
    return 0;
  }
};
 ...
 * @returns Boolean indicating if the user is interested in the event
 */
export const isUserInterestedInEvent = async (eventId: string, userId: string): Promise<boolean> => {
  try {
    ...
    console.error("Unexpected error checking if user is interested in event:", error);
    return false;
  }
};
