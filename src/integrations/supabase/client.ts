...
export const markEventInterest = async (eventId: string, userId: string): Promise<boolean> => {
  try {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ events_upcoming: supabase.from('profiles').select('events_upcoming').eq('id', userId).single().then(res => (res.data?.events_upcoming || 0) + 1) })
      .eq('id', userId);
...
export const removeEventInterest = async (eventId: string, userId: string): Promise<boolean> => {
  try {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ 
        events_upcoming: supabase.from('profiles').select('events_upcoming').eq('id', userId).single().then(res => Math.max(0, (res.data?.events_upcoming || 0) - 1))
      })
      .eq('id', userId);
...
export const checkInUserToEvent = async (eventId: string, userId: string): Promise<boolean> => {
  try {
    ...
    const { error: updateError } = await supabase
      .from('profiles')
      .update({ 
        events_attended: supabase.from('profiles').select('events_attended').eq('id', userId).single().then(res => (res.data?.events_attended || 0) + 1) 
      })
      .eq('id', userId);
