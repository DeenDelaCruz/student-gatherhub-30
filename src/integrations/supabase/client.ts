...
export const getInformationOfficers = async (): Promise<any[]> => {
  try {
    // Get all user_ids with information_officer role and join with their profile data
    const { data, error } = await supabase
      .from('user_roles')
      .select(`
        user_id,
    ...
    if (error) {
      console.error('Error in getInformationOfficers query:', error);
      throw error;
    }
    
    // Format the data to match the expected structure in the Admin component
    const formattedData = data?.map(item => ({
      user_id: item.user_id,
      profiles: item.profiles
    })) || [];
    
    console.log('Information officers fetched:', formattedData);
